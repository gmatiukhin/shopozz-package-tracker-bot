...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Please provide a tracking number using /track command")
...
async def invalid_tracking_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    assert update.effective_chat is not None
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Sorry, your track number is invalid.")

async def remove_tracking_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args and update.effective_chat is not None:
        chat_id = update.effective_chat.id
        tracking_number = context.args[0]
        tracklist.remove(chat_id, tracking_number)

        await context.bot.send_message(chat_id=update.effective_chat.id, text="Done, I am no longer tracking your package.")
...
if __name__ == '__main__':
    ...
    new_tracking_number_handler = CommandHandler("track", new_tracking_number, filters.Regex(r"[A-Z]{2}\d{9}RU"))
    ...
    remove_tracking_number_handler = CommandHandler("untrack", remove_tracking_number, filters.Regex(r"[A-Z]{2}\d{9}RU"))
